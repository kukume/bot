...
import io.ktor.http.*
import me.kuku.common.entity.StepEntity
import me.kuku.common.ktor.client
import me.kuku.common.ktor.renderCookieHeader
import me.kuku.common.utils.RegexUtils
import me.kuku.common.utils.aes
import me.kuku.common.utils.md5
import me.kuku.common.utils.toUrlDecode
import java.time.LocalDate
import java.time.LocalDateTime
import java.time.format.DateTimeFormatter
import java.util.*

data class LeiXinStepLogin(
    val leiXinCookie: String,
    val leXinUserid: String,
    val leiXinAccessToken: String
)

object LeXinStepLogic {

    suspend fun login(phone: String, password: String): LeiXinStepLogin {
        ...
        return if (jsonNode["code"].asInt() == 200) {
            ...
            LeiXinStepLogin(cookie, jsonNode.get("data")["userId"].asText(), jsonNode.get("data")["accessToken"].asText())
        } else error(jsonNode["msg"].asText())
    }

    suspend fun modifyStepCount(stepEntity: StepEntity, step: Int) {
        if (stepEntity.leXinCookie == null) error("未绑定乐心运动，操作失败")
...
data class XiaomiStepLogin(val miLoginToken: String)

object XiaomiStepLogic {

    private const val ua = "MiFit6.14.0 (24129PN74C; Android 16; Density/2.75)"

    private fun HttpMessageBuilder.defaultHeader() {
        headers.append("x-hm-ekv", "1")
        headers.append("app_name", "com.xiaomi.hm.health")
        headers.append("appname", "com.xiaomi.hm.health")
        headers.append("appplatform", "android_phone")
        headers.append("User-Agent", ua)
    }

    suspend fun login(phone: String, password: String): XiaomiStepLogin {
        val map = mutableMapOf("emailOrPhone" to "+86$phone", "password" to password,
        ...
        val value = map.toList().formUrlEncode()
        val aesValue = value.aes("xeNtBVqzDc6tuNTh", "MAAAYAAAAAAAAABg")

        val response = client.post("https://api-user.zepp.com/v2/registrations/tokens") {
            setBody(aesValue)
        ...
        val ssMap = mapOf("app_name" to "com.xiaomi.hm.health", "country_code" to "CN", "code" to access,
            "device_id" to "37:83:85:5a:e8:93", "device_model" to "android_phone", "app_version" to "6.14.0",
    ...
    suspend fun modifyStepCount(stepEntity: StepEntity, step: Int) {
        val token = stepEntity.miLoginToken ?: error("未绑定小米运动，操作失败")
        modifyStepCount(token, step)
    }

    suspend fun modifyStepCount(token: String, step: Int) {
