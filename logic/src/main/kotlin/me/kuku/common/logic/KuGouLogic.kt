...
import io.ktor.http.*
import me.kuku.common.entity.KuGouEntity
...
object KuGouLogic {
    ...
    suspend fun checkQrcode(kuGouQrcode: KuGouQrcode): KuGouLogin {
        ...
        return when (dataStatus) {
            ...
            4 -> {
                ...
                KuGouLogin(token, userid, kuGoo!!, kuGouQrcode.mid)
            }
    ...
    suspend fun verifyCode(phone: String, code: String, mid: String): KuGouLogin {
        val time = clientTime()
        val map = mutableMapOf(
            "appid" to "3116",
            "clientver" to "10",
        ...
        return if (jsonNode["error_code"].asInt() == 0) {
            ...
            KuGouLogin(token!!, userid.toLong(), kuGoo, mid)
        }
        else error(jsonNode["data"].asText())
    }

    suspend fun login(username: String, password: String, mid: String?): KuGouLogin {
        val newMid = mid ?: mid()
        ...
        return when (jsonNode["errorCode"]?.asInt()){
            ...
            null -> {
                ...
                KuGouLogin(token, userid, kuGoo!!, mid)
            }
...
data class KuGouLogin(val token: String, val userid: Long, val kuGoo: String, val mid: String?)