...
import me.kuku.common.entity.BaiduEntity
import me.kuku.common.entity.BaiduPojo
...
object BaiduLogic {
    ...
    suspend fun checkQrcode(baiduQrcode: BaiduQrcode): BaiduPojo {
        val jsonNode = try {
    ...
    suspend fun tieBaSign(baiduEntity: BaiduEntity) {
        ...
        if (sToken == null) saveSToken(baiduEntity, url)
        val userAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36"
        val baiduResponse = client.get("https://www.baidu.com/") {
            userAgent(userAgent)
        }
        val headers = mapOf("user-agent" to userAgent, "cookie" to baiduEntity.teiBaCookie() + baiduResponse.setCookie().renderCookieHeader())
