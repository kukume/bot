...
import me.kuku.common.entity.MiHoYoEntity
import me.kuku.common.exception.qrcodeNotScanned
import me.kuku.common.ktor.*
...
object MiHoYoLogic {
    ...
    private fun MiHoYoFix.append() {
        val jsonNode = Jackson.readTree(Jackson.writeValueAsString(this))
        for (entry in jsonNode.properties()) {
            val key = entry.key
    ...
    private fun MiHoYoFix.commonAppend(ds: MiHoYoDs) {
        val fix = this
        builder.headers["x-rpc-device_id"] = fix.device.replace("-", "")
        builder.headers["x-rpc-client_type"] = ds.clientType
        builder.headers["x-rpc-app_version"] = ds.appVersion
        builder.headers["X-Rpc-Device_fp"] = fix.fp
        builder.headers["DS"] = ds.ds
    }
    ...
    private fun MiHoYoFix.appAppend() {
        val ds = ds()
        commonAppend(ds)
        builder.headers["User-Agent"] = "Mozilla/5.0 (iPhone; CPU iPhone OS 14_0_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) miHoYoBBS/${ds.appVersion}"
    }
    ...
    private fun MiHoYoFix.appNewAppend(data: Map<String, Any>? = null) {
        val ds = newDs(data)
        commonAppend(ds)
        builder.referer("https://app.mihoyo.com")
        builder.headers["User-Agent"] = "Mozilla/5.0 (iPhone; CPU iPhone OS 14_0_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) miHoYoBBS/${ds.appVersion}"
    }
    ...
    private fun MiHoYoFix.webAppend() {
        val ds = webDs()
        commonAppend(ds)
        builder.referer("https://www.miyoushe.com/")
        builder.headers["user-agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36"
    }
    ...
    suspend fun qrcodeLogin2(qrcode: MiHoYoQrcode): MiHoYoLogin {
        ...
        return when (val status = data["status"].asText()) {
            ...
            "Confirmed" -> {
                ...
                MiHoYoLogin(qrcode.fix, cookie, userInfo["aid"].asText(), userInfo["mid"].asText(), token, null)
            }
            else -> error("米游社登陆失败，未知的状态：$status")
        }
    }

    suspend fun login(account: String, password: String): MiHoYoLogin {
        ...
        if (code == -3101) {
            ...
            val rr = DaMaGouLogic.geeTest(gt, challenge,"https://static.mohoyo.com")
            val aiGis = "$sessionId;" + Base64.encode("""
        ...
        val login = MiHoYoLogin(fix, null, aid, mid, token, ticket)
        val sToken = sToken(login)
        login.sToken = sToken
        return login
    }

    suspend fun webLogin(account: String, password: String): MiHoYoLogin {
        val fix = MiHoYoFix()
        val rsaKey = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDDvekdPMHN3AYhm/vktJT+YJr7cI5DcsNKqdsx5DZX0gDuWFuIjzdwButrIYPNmRJ1G8ybDIF7oDW2eEpm5sMbL9zs9ExXCdvqrn51qELbqj0XxtMTIpaCHFSI50PfPpTFV9Xt/hmyVwokoOXFlAEgCn+QCgGs52bFoYMtyi+xEQIDAQAB"
        ...
        if (code == -3101) {
            ...
            val challenge = captchaJsonNode["challenge"].asText()
            ...
            DaMaGouLogic.geeTestV4(captchaId, riskType, challenge, "https://user.mihoyo.com/")
        ...
        cookie += finaCookie
        return MiHoYoLogin(MiHoYoFix(), cookie, aid, mid, null, null)
    }

    context(builder: HttpRequestBuilder)
    private fun MiHoYoFix.loginHeader() {
        val fix = this
        builder.referer("https://user.mihoyo.com/")
        builder.origin("https://user.mihoyo.com")
        val headers = builder.headers
        headers["x-rpc-app_id"] = fix.app
        headers["X-Rpc-Client_type"] = "4"
        headers["X-Rpc-Device_fp"] = fix.fp
        headers["X-Rpc-Device_id"] = fix.device
        headers["X-Rpc-Device_model"] = "Chrome%20132.0.0.0"
        headers["X-Rpc-Device_name"] = "Chrome"
        headers["x-rpc-device_os"] = "Windows%2010%2064-bit"
        headers["X-Rpc-Game_biz"] = "account_cn"
        headers["x-rpc-lifecycle_id"] = "011bddb4e4"
        headers["X-Rpc-Mi_referrer"] = "https://user.mihoyo.com/"
        headers["x-rpc-sdk_version"] = "2.35.0"
        headers["X-Rpc-Source"] = "accountWebsite"
        headers["user-agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36"
        headers["cookie"] = "_MHYUUID=${fix.device}; DEVICEFP_SEED_ID=${RandomUtils.num(16)}; DEVICEFP_SEED_TIME=${System.currentTimeMillis()}; DEVICEFP=${fix.fp};MIHOYO_LOGIN_PLATFORM_LIFECYCLE_ID=011bddb4e4;"
    }
    ...
    suspend fun sign(miHoYoEntity: MiHoYoEntity) {
        ...
        for (obj in jsonArray) {
            ...
            when (jsonNode["retcode"].asInt()) {
                0, -5003 -> {
                    ...
                    if (gt.isNotEmpty()) {
                        val challenge = data["challenge"].asText()
                        val rr = DaMaGouLogic.geeTest(gt, challenge, "https://webstatic.mihoyo.com/")
    ...
    private suspend fun hubVerifyGeeTest(miHoYoEntity: MiHoYoEntity) {
        ...
        val challenge = data["challenge"].asText()
        val gt = data["gt"].asText()
        val rr = DaMaGouLogic.geeTest(gt, challenge, "https://bbs.mihoyo.com")
    ...
    suspend fun sToken(miHoYoEntity: MiHoYoLogin): String {
