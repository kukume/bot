...
import me.kuku.common.entity.ECloudEntity
...
object ECloudLogic  {
    ...
    suspend fun login(username: String, password: String): ECloudLogin {
        ...
        val response = client.submitForm("https://open.e.189.cn/api/logbox/oauth2/loginSubmit.do",
            parameters {
                ...
                append("appKey", "cloud")
                append("accountType", "01")
                append("userName", pre + Base64.decode(username.rsaEncrypt(pubKey)).hex())
                append("epd", pre + Base64.decode(password.rsaEncrypt(pubKey)).hex())
                append("captchaType", "")
                append("validateCode", "")
        ...
        return ECloudLogin(eCookie, resultCookie)
    }
...
data class ECloudLogin(val eCookie: String, val cookie: String)
