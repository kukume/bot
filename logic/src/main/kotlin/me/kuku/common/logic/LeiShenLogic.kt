...
object LeiShenLogic {
    ...
    suspend fun login(phone: String, password: String): LeiShenLogin {
        val tencent = DeathByCaptchaLogic.tencent(198759457, "https://www.leigod.com/")
        val params = """
            {"rand_str":"${tencent.randStr}","behavior_ticket":"${tencent.ticket}","os_type":4,"password":"${password.md5()}","mobile_num":"$phone","src_channel":"guanwang","country_code":86,"username":"$phone","lang":"zh_CN","region_code":1,"ts":"${System.currentTimeMillis() / 1000}"}
        """.trimIndent().toJsonNode() as ObjectNode
        val sortedFields = params.properties().asSequence()
        ...
        val md5 = convert.md5()
        params.put("sign", md5)
        val jsonNode = client.post("https://webapi.leigod.com/api/auth/login/v2") {
            setJsonBody(params)
        ...
        return LeiShenLogin(accountToken, nnToken, expiryTime, phone, password)
    }
