...
object BiliBiliLogic {
    ...
    suspend fun videoByBvId(param: String): BiliBiliVideo {
        val bvRegex = "BV[A-Za-z0-9]{10}".toRegex()
        var bvId = bvRegex.find(param)?.groupValues?.get(0)
        if (bvId == null) {
            "https://b23\\.tv/[a-zA-Z0-9]+".toRegex().find(param)?.groupValues?.get(0)?.let {
                val response = client.get(it)
                if (response.status == HttpStatusCode.Found) {
                    val bvUrl = response.headers["Location"]!!
                    bvId = bvRegex.find(bvUrl)?.groupValues?.get(0)
                }
            }
        }
        if (bvId == null) {
            error("not found bvId")
        }
        ...
        return if (response.status != HttpStatusCode.OK) {
            ...
            val pic = videoData["pic"].asText()
            val ownerName = videoData["owner"]["name"].asText()
            val ownerMid = videoData["owner"]["mid"].asText()
            ...
            val videoUrl = videoJsonNode["data"]["dash"]["video"][0]["baseUrl"].asText()
            val videoFile = File("tmp${File.separator}$bvId.mp4")
            segmentsDownload(videoFile, videoUrl) {
                referer(htmlUrl)
            }
            val audioUrl = videoJsonNode["data"]["dash"]["audio"][0]["baseUrl"].asText()
            ...
            BiliBiliVideo(title, desc, pic, ownerName, ownerMid, face, file)
        }
    }
    ...
    suspend fun loginByQr2(qrcode: BiliBiliQrcode): BiliBiliLogin {
        ...
        return when(data["code"].asInt()) {
            ...
            0 -> {
                ...
                BiliBiliLogin(cookie + fingerCookie, userid, token)
            }
    ...
    suspend fun share(biliBiliEntity: BiliBiliEntity, aid: String) {
        val jsonNode = client.submitForm("https://api.bilibili.com/x/share/finish",
            parameters {
                append("access_key", "753daf8b2a14c01655c1f07eb42bbb21CjD3j87eVdHohxnEY6LfZ75TYHumLBskZDkPzqH-I-_rquWaU5xzUyWc_uCnZ9wW1eASVlV4eXlFT3hYMzRqNTFzUDBXdEF2MFl3SjNoWUJWazZfdm55dlh5TEx0YnNnWXFmcTc2dkR2RjdwSDN2TUJUbnJ4M19MYkdMNnNXanRSemdXNnk5UVBBIIEC")
                append("appkey", "1d8b6e7d45233436")
                append("build", "8330200")
                append("c_locale", "zh_CN")
                append("channel", "vivo")
                append("disable_rcmd", "0")
                append("from_spmid", "tm.recommend.0.0")
                append("link", "https://b23.tv/p56PwKj")
                append("mobi_app", "android")
                append("object_extra_fields", """{"share_channel_list":["WEIXIN","WEIXIN_MONMENT","SYS_DOWNLOAD","QQ","QZONE","SINA","biliDynamic","biliIm","FIRE","PIC","MARK_POINT","COPY","GENERIC"],"share_channel_mark":{"QQ":"上次分享"}}""")
                append("oid", "114006584986722")
                append("panel_type", "1")
                append("platform", "android")
                append("s_locale", "zh_CN")
                append("share_channel", "QQ")
                append("share_id", "main.ugc-video-detail.0.0.pv")
                append("share_origin", "vinfo_share")
                append("share_session_id", "6aff13b0-e78f-4f97-a944-86dad977e86f")
                append("sid", "28402126718")
                append("spm_id", "united.player-video-detail.0.0")
                append("statistics", """{"appId":1,"platform":3,"version":"8.33.0","abtest":""}""")
                append("success", "true")
                append("ts", "1739689006")
                append("sign", "f4b04fd9c3b55ea7dec924f2d39ee10b")
            }) {
            cookieString(biliBiliEntity.cookie)
        }.body<JsonNode>()
...
object Wbi {
    ...
    private var t: LocalDateTime = LocalDateTime.now(ZoneId.of("+8"))

    suspend fun enc(params: String): String {
        val split1 = params.split("&")
        val map = mutableMapOf<String, Any>()
        for (split in split1) {
            val split2 = split.split("=")
            map[split2[0]] = split2.getOrNull(1) ?: ""
        }
        return enc(map)
    }

    suspend fun enc(params: Map<String, Any>): String {
        val now = LocalDateTime.now(ZoneId.of("+8"))
        if (o.isEmpty() || a.isEmpty() || now.dayOfMonth != t.dayOfMonth) {
            val jsonNode = client.get("https://api.bilibili.com/x/web-interface/nav").body<JsonNode>()
            val data = jsonNode["data"]["wbi_img"]
            o = data["img_url"].asText().split('/').last().removeSuffix(".png")
            a = data["sub_url"].asText().split('/').last().removeSuffix(".png")
            t = now
        }
        return enc(o, a, params)
    }

    private fun enc(o: String, a: String, params: Map<String, Any>): String {
        val r = o + a
        val n = StringBuilder()
        mixinKeyEncTab.forEach {
            n.append(r[it])
        }
        ...
        val m = sortedMap.map { URLEncoder.encode(it.key, "utf-8") + "=" + URLEncoder.encode(it.value.toString(), "utf-8") }.joinToString("&")
        val wRid = (m + s).md5()
        return "$m&w_rid=$wRid"
    }
